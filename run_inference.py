...
def inference(model_config, eval_config,
    ...
    path_bev=""
    path_img=""
    files=os.listdir(path_bev)   #查找路径下的所有的文件夹及文件
    for filee in files:
        f=str(path_bev+filee)    #使用绝对路径
        if os.path.isdir(f):  #判断是文件夹还是文件
            if not os.listdir(f):  #判断文件夹是否为空
                for filee in files:
                    f=str(path_img+filee)    #使用绝对路径
                    if os.path.isdir(f):  #判断是文件夹还是文件
                        if not os.listdir(f):  #判断文件夹是否为空
                            break 
                        else:
                            model_config.path_drop_probabilities = [0, 1.0]
            else:
                for filee in files:
                    f=str(path_img+filee)    #使用绝对路径
                    if os.path.isdir(f):  #判断是文件夹还是文件
                        if not os.listdir(f):  #判断文件夹是否为空
                            model_config.path_drop_probabilities = [1.0, 0]
                        else:
...
def main(_):
    ...
    experiment_config_path = ammf.root_dir() + '/data/outputs/' +\
