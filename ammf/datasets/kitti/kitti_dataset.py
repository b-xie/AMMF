...
from ammf.datasets.kitti.kitti_utils import KittiUtils
...
class KittiDataset:
    def __init__(self, dataset_config):
        ...
        #2.Get possible data splits from txt files in dataset folder
        possible_splits = []
    ...
    @property
    def rgb_seg_dir(self):
        return self.seg_dir
    @property
    def rgb_label_seg_dir(self):
        return self.label_seg_dir
    ...
    def _set_up_directories(self):
        ...
        self.seg_dir = self._data_split_dir + '/image_seg_' + str(self._cam_idx)
        ...
        self.label_seg_dir = self.dataset_dir + \
            '/training/label_seg_' + str(self._cam_idx)
    ...
    def get_rgb_seg_path(self, sample_name):
        return self.rgb_seg_dir + '/' + sample_name + '.png'
    def get_rgb_label_seg_path(self, sample_name):
        return self.rgb_label_seg_dir + '/' + sample_name + '.png'
    ...
    def load_samples(self, indices):
        ...
        for sample_idx in indices:
            ...
            seg_input=0
            label_seg_input=0
            seg_filelist = os.listdir('/media/bangquanxie/4FCF996C7FA0ED8D/Kitti/object/training/image_seg_2')
            if int(sample_name) < len(seg_filelist):
                ...
                cv_bgr_label_seg = plt.imread(self.get_rgb_label_seg_path(sample_name))
                print(cv_bgr_seg.shape)
                print("cv_bgr_seg.shape")
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size >= self.num_samples:
            ...
            samples_in_batch.extend(self.load_samples(np.arange(start, end)))
            #samples_in_batch.extend(self.load_samples_seg(np.arange(start, 384)))
