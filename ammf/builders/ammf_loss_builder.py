...
def build(model, prediction_dict):
    ...
    ammf_box_rep = model._config.ammf_config.ammf_box_representation
    #1. _build_cls_off_ang_loss
    if ammf_box_rep in ['box_3d', 'box_4ca']:#b:one
        # Boxes with angle vector output
        losses_output = _build_cls_off_ang_loss(model, prediction_dict)
    #2. _build_cls_off_loss
    elif ammf_box_rep in ['box_8c', 'box_8co', 'box_4c']:#b:two
        losses_output = _build_cls_off_loss(model, prediction_dict)

    #3.bqx:road:_build_cls_off_road_loss    

    else:
        raise ValueError('Invalid box representation', ammf_box_rep)
...
def _build_cls_off_ang_loss(model, prediction_dict):
    """Builds classification, offset, and angle vector losses.#3个损失！
    ...
    mb_cls_logits = prediction_dict[model.PRED_MB_CLASSIFICATION_LOGITS]#cls
    mb_cls_softmax = prediction_dict[model.PRED_MB_CLASSIFICATION_SOFTMAX]#bqx:cls have two 
    ...
    with tf.variable_scope('ammf_gt_angle_vectors'):
        mb_angle_vectors_gt = \
            orientation_encoder.tf_orientation_to_angle_vector(
                mb_orientations_gt)

    # Losses
    with tf.variable_scope('ammf_losses'):
        ...
        with tf.variable_scope('ammf_loss'):
            #bqx:road
            ammf_loss = cls_loss + final_reg_loss
            tf.summary.scalar('ammf_loss', ammf_loss)
    ...
    losses_output[KEY_ammf_LOSS] = ammf_loss
