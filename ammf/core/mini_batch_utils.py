...
from ammf.core.mini_batch_preprocessor import MiniBatchPreprocessor
from ammf.core.minibatch_samplers import balanced_positive_negative_sampler
...
class MiniBatchUtils:
    def __init__(self, dataset):
        ...
        self.p1_neg_iou_range = [self.p1_iou_thresholds.neg_iou_lo,
                                  self.p1_iou_thresholds.neg_iou_hi]
        self.p1_pos_iou_range = [self.p1_iou_thresholds.pos_iou_lo,
        ...
        self.ammf_neg_iou_range = [self.ammf_iou_thresholds.neg_iou_lo,
                                   self.ammf_iou_thresholds.neg_iou_hi]
        self.ammf_pos_iou_range = [self.ammf_iou_thresholds.pos_iou_lo,
    ...
    def preprocess_p1_mini_batches(self, indices):
        """Generates p1 mini batch info for the kitti dataset

            Preprocesses data and saves data to files.
            Each file contains information that is used to feed
            to the network for p1 training.
        """
    ...
    def sample_p1_mini_batch(self, anchor_ious):
        """ Samples a mini batch to train the p1 with preconfigured
        ...
        return self.sample_mini_batch(anchor_ious,
                                      self.p1_mini_batch_size,
                                      self.p1_neg_iou_range,
                                      self.p1_pos_iou_range)

    def sample_ammf_mini_batch(self, anchor_ious):
        """ Samples a mini batch to train ammf with preconfigured
