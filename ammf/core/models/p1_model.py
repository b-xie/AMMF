...
class p1Model(model.DetectionModel):
    ...
    PL_GROUND_PLANE_PSE = 'ground_plane_pse'


    #  placeholder for sparse_pooling_inputs
    ...
    ##############################
    #2. Keys for Predictions
    ##############################
    PRED_ANCHORS = 'p1_anchors'

    PRED_MB_OBJECTNESS_GT = 'p1_mb_objectness_gt'
    PRED_MB_OFFSETS_GT = 'p1_mb_offsets_gt'
  

    PRED_MB_MASK = 'p1_mb_mask'
    PRED_MB_OBJECTNESS = 'p1_mb_objectness'
    PRED_MB_OFFSETS = 'p1_mb_offsets'

    PRED_TOP_INDICES = 'p1_top_indices'
    PRED_TOP_ANCHORS = 'p1_top_anchors'
    PRED_TOP_OBJECTNESS_SOFTMAX = 'p1_top_objectness_softmax'

    ##############################
    #3. Keys for Loss
    ##############################
    LOSS_p1_OBJECTNESS = 'p1_objectness_loss'
    LOSS_p1_REGRESSION = 'p1_regression_loss'
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        #1.1 Sets model configs (_config)
        super(p1Model, self).__init__(model_config)
        ...
        #1.3 p1 config
        p1_config = self._config.p1_config
        if hasattr(self._config.p1_config,'p1_use_sparse_pooling'):
            if dataset.config.output_indices:
                ...
                self._config.p1_config.p1_use_sparse_pooling = False
        else:
            self._config.p1_config.p1_use_sparse_pooling = False


        self._proposal_roi_crop_size = \
            [p1_config.p1_proposal_roi_crop_size] * 2
        self._fusion_method = p1_config.p1_fusion_method

        if self._train_val_test in ["train", "val"]:
            self._nms_size = p1_config.p1_train_nms_size
        else:
            self._nms_size = p1_config.p1_test_nms_size

        self._nms_iou_thresh = p1_config.p1_nms_iou_thresh

        #2. Feature Extractor Nets
        ...
        self.dataset.train_on_all_samples = self._train_on_all_samples
            ...
            #self._path_drop_probabilities[0] = 0.9
            #self._path_drop_probabilities[1] = 0.9
    #2.
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('bev_pse_input'):
            # Placeholder for BEV image input, to be filled in with feed_dict
            bev_pse_input_placeholder = self._add_placeholder(tf.float32, bev_dims,
                                                          self.PL_BEV_PSE_INPUT)

            self._bev_pse_input_batches = tf.expand_dims(
                bev_pse_input_placeholder, axis=0)

            self._bev_preprocessed_pse = \
                self._bev_feature_extractor.preprocess_input(
                    self._bev_pse_input_batches, self._bev_pixel_size)

            # Summary Images
            bev_summary_images_pse = tf.split(
                bev_pse_input_placeholder, self._bev_depth, axis=2)
            tf.summary.image("bev_maps_pse", bev_summary_images_pse,
                             max_outputs=self._bev_depth)
    ...
    #4
    def _set_up_feature_extraction_phase(self):
        ...
        if self._config.p1_config.p1_sparse_pooling_after_vgg:
            ...
            if self._config.p1_config.p1_dual_sparse_pooling_after_vgg:
                bv_index_indicator = np.zeros((1,3))
            else:
                print('WZN: only pooling img to bev after vgg')
                bv_index_indicator = None
            self._fusion_feature_extractor = FusionVggPyr(self._config.layers_config.bev_feature_extractor.bev_vgg_pyr,  #fusion feature!!！
                    self._config.layers_config.img_feature_extractor.img_vgg_pyr, self.M_tf_vgg, #bqx
                    img_index_flip=self.placeholders[self.PL_IMG_POOL_IJ_VGG],bv_index=bv_index_indicator,bep_index=None)
        ...
        #  Here is where he shrinks the channels of the features.
    ...
    def build(self):
        ...
        self._set_up_feature_extraction_phase()
        ...
        if not (self._path_drop_probabilities[0] ==
            ...
            with tf.variable_scope('p1_path_drop'):
        ...
        # The learned attention map for feature vector fusion
        with tf.variable_scope('proposal_roi_fusion'):
            rpn_fusion_out = None
            if self._fusion_method == 'AF':
                
                rpn_fusion_out = learned_fusion.AF(bev_proposal_rois, img_proposal_rois)

            elif self._fusion_method == 'CF':
                rpn_fusion_out = learned_fusion.CF(bev_proposal_rois, img_proposal_rois)
            else:
        ...
        with tf.variable_scope('anchor_predictor', 'ap', [p1_fusion_out]):
            ...
            # Parse p1 layers config
            layers_config = self._config.layers_config.p1_config
        ...
        with tf.variable_scope('histograms_p1'):
        ...
        with tf.variable_scope('mini_batch'):
            ...
            mini_batch_mask, _ = \
                mini_batch_utils.sample_p1_mini_batch(all_ious_gt)

        # ROI summary images
        p1_mini_batch_size = \
            self.dataset.kitti_utils.mini_batch_utils.p1_mini_batch_size
        with tf.variable_scope('bev_p1_rois'):
            ...
            tf.summary.image('bev_p1_rois',
                             bev_input_roi_summary_images[-1],
                             max_outputs=p1_mini_batch_size)

        with tf.variable_scope('img_p1_rois'):
            # ROIs on image input
        ...
        with tf.variable_scope('one_hot_classes'):
            ...
            # Object / Not Object
            min_pos_iou = \
                self.dataset.kitti_utils.mini_batch_utils.p1_pos_iou_range[0]
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        bev_pse_input = sample.get(constants.KEY_BEV_PSE_INPUT)
        ...
        self._placeholder_inputs[self.PL_BEV_PSE_INPUT] = bev_pse_input
        ...
        self.sample_info['p1_mini_batch'] = anchors_info
    ...
    def loss(self, prediction_dict):
        ...
        loss_dict = {
            self.LOSS_p1_OBJECTNESS: objectness_loss,
            self.LOSS_p1_REGRESSION: localization_loss,
        }
