...
from ammf.core import evaluator_utils
from ammf.core import summary_utils
from ammf.core import trainer_utils
...
KEY_SUM_p1_OBJ_LOSS = 'sum_p1_obj_loss'
KEY_SUM_p1_REG_LOSS = 'sum_p1_reg_loss'
KEY_SUM_p1_TOTAL_LOSS = 'sum_p1_total_loss'
KEY_SUM_p1_OBJ_ACC = 'sum_p1_obj_accuracy'

KEY_SUM_ammf_CLS_LOSS = 'sum_ammf_cls_loss'
KEY_SUM_ammf_REG_LOSS = 'sum_ammf_reg_loss'
KEY_SUM_ammf_TOTAL_LOSS = 'sum_ammf_total_loss'
KEY_SUM_ammf_LOC_LOSS = 'sum_ammf_loc_loss'
KEY_SUM_ammf_ANG_LOSS = 'sum_ammf_ang_loss'
KEY_SUM_ammf_CLS_ACC = 'sum_ammf_cls_accuracy'
...
class Evaluator:
    ...
    def __init__(self,
        ...
        self.full_model = isinstance(self.model, ammfModel)
        ...
        #2. Create a variable tensor to hold the global step
        ...
        #3. The model should return a dictionary of predictions
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        # p1 average losses dictionary
        if validation:
            eval_p1_losses = self._create_p1_losses_dict()
        ...
        if self.full_model:
            ...
            box_rep = self.model_config.ammf_config.ammf_box_representation
            if box_rep not in ['box_3d', 'box_8c', 'box_8co',
                               'box_4c', 'box_4ca']:
                raise ValueError('Invalid box representation {}.'.
                                 format(box_rep))

            ammf_predictions_dir = predictions_base_dir + \
                "/final_predictions_and_scores/{}/{}".format(
                    data_split, global_step)
            trainer_utils.create_dir(ammf_predictions_dir)

            if box_rep in ['box_8c', 'box_8co', 'box_4c', 'box_4ca']:
                ammf_box_corners_dir = predictions_base_dir + \
                    "/final_boxes_{}_and_scores/{}/{}".format(
                        box_rep, data_split, global_step)
                trainer_utils.create_dir(ammf_box_corners_dir)

            # ammf average losses dictionary
            eval_ammf_losses = self._create_ammf_losses_dict()
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            p1_file_path = prop_score_predictions_dir + "/{}.txt".format(
                sample_name)

            if self.full_model:
                ammf_file_path = ammf_predictions_dir + \
                    "/{}.txt".format(sample_name)

                if box_rep in ['box_8c', 'box_8co', 'box_4c', 'box_4ca']:
                    ammf_box_corners_file_path = ammf_box_corners_dir + \
                        '/{}.txt'.format(sample_name)
            ...
            if validation:
                ...
                p1_objectness_loss = eval_losses[p1Model.LOSS_p1_OBJECTNESS]
                p1_regression_loss = eval_losses[p1Model.LOSS_p1_REGRESSION]

                self._update_p1_losses(eval_p1_losses,
                                        p1_objectness_loss,
                                        p1_regression_loss,
                                        eval_total_loss,
                                        global_step)
                ...
                proposals_and_scores = \
                    self.get_p1_proposals_and_scores(predictions)
                np.savetxt(p1_file_path, proposals_and_scores, fmt='%.3f')

                # Save predictions
                predictions_and_scores = \
                    self.get_ammf_predicted_boxes_3d_and_scores(predictions,
                                                                box_rep)
                np.savetxt(ammf_file_path, predictions_and_scores, fmt='%.5f')

                if self.full_model:
                    if box_rep in ['box_3d', 'box_4ca']:
                        self._update_ammf_box_cls_loc_orient_losses(
                            eval_ammf_losses,
                            eval_losses,
                            eval_total_loss,
                            global_step)
                        ...
                        self._update_ammf_box_cls_loc_losses(
                            eval_ammf_losses,
                            eval_losses,
                            eval_total_loss,
                            global_step)
                    ...
                    if box_rep != 'box_3d':
                        ...
                        predicted_box_corners_and_scores = \
                            self.get_ammf_predicted_box_corners_and_scores(
                                predictions, box_rep)
                        np.savetxt(ammf_box_corners_file_path,
                                   predicted_box_corners_and_scores,
                                   fmt='%.5f')
                ...
                dataset = DatasetBuilder.build_kitti_dataset(self.dataset_config ,
                                                 use_defaults=False)
                ...
                # Load predictions from files
                    ...
                    if prediction_class_indices.all()< 0.4:#class主要靠IMG,如果它低了，说明img不行了。
                        self.model_config.path_drop_probabilities = [0.9, 0]
                        print("bqx:use bev2")
                    print("bqx:use imgimgimgimg")    
                print("bqx:use imgimgimgimgimg")  
                '''
        ...
        if validation:
            self.save_proposal_losses_results(eval_p1_losses,
                                              num_valid_samples,
                                              global_step,
    ...
    def run_latest_checkpoints(self):
        ...
        #1. Load the latest checkpoints available
    ...
    def _update_p1_losses(self,
        ...
        Args:
            eval_p1_losses: A dictionary containing all the average
                losses.
            p1_objectness_loss: A scalar loss of p1 objectness.
            p1_regression_loss: A scalar loss of p1 objectness.
            eval_total_loss: A scalar loss of p1 total loss.
            global_step: Global step at which the metrics are computed.
        """

        if self.full_model:
            # The full model total_loss will be the sum of p1 and ammf
            # so calculate the total p1 loss instead
            p1_total_loss = p1_objectness_loss + p1_regression_loss
        else:
            p1_total_loss = eval_total_loss

        print("Step {}: Eval p1 Loss: objectness {:.3f}, "
        ...
        sum_p1_obj_loss = eval_p1_losses[KEY_SUM_p1_OBJ_LOSS]
        sum_p1_reg_loss = eval_p1_losses[KEY_SUM_p1_REG_LOSS]
        sum_p1_total_loss = eval_p1_losses[KEY_SUM_p1_TOTAL_LOSS]

        sum_p1_obj_loss += p1_objectness_loss
        sum_p1_reg_loss += p1_regression_loss
        sum_p1_total_loss += p1_total_loss

        # update the losses sums
        eval_p1_losses.update({KEY_SUM_p1_OBJ_LOSS:
                                sum_p1_obj_loss})

        eval_p1_losses.update({KEY_SUM_p1_REG_LOSS:
                                sum_p1_reg_loss})

        eval_p1_losses.update({KEY_SUM_p1_TOTAL_LOSS:
                                sum_p1_total_loss})

    def _update_ammf_box_cls_loc_orient_losses(self,
                                               eval_ammf_losses,
                                               eval_losses,
                                               eval_total_loss,
                                               global_step):
        ...
        Args:
            eval_ammf_losses: A dictionary containing all the average
                losses.
        ...
        """
        # Get the loss sums from the losses dict
        sum_ammf_cls_loss = eval_ammf_losses[KEY_SUM_ammf_CLS_LOSS]
        sum_ammf_reg_loss = eval_ammf_losses[KEY_SUM_ammf_REG_LOSS]
        sum_ammf_total_loss = eval_ammf_losses[KEY_SUM_ammf_TOTAL_LOSS]

        # for the full model, we expect a total of 4 losses
        assert (len(eval_losses) > 2)
        ammf_classification_loss = \
            eval_losses[ammfModel.LOSS_FINAL_CLASSIFICATION]
        ammf_regression_loss = \
            eval_losses[ammfModel.LOSS_FINAL_REGRESSION]

        ammf_localization_loss = \
            eval_losses[ammfModel.LOSS_FINAL_LOCALIZATION]
        ammf_orientation_loss = \
            eval_losses[ammfModel.LOSS_FINAL_ORIENTATION]

        sum_ammf_cls_loss += ammf_classification_loss
        sum_ammf_reg_loss += ammf_regression_loss
        sum_ammf_total_loss += eval_total_loss

        # update the losses sums
        eval_ammf_losses.update({KEY_SUM_ammf_CLS_LOSS:
                                 sum_ammf_cls_loss})

        eval_ammf_losses.update({KEY_SUM_ammf_REG_LOSS:
                                 sum_ammf_reg_loss})

        eval_ammf_losses.update({KEY_SUM_ammf_TOTAL_LOSS:
                                 sum_ammf_total_loss})

        # Only add localization and orientation losses if valid
        # (greater than 0)
        if (ammf_localization_loss > 0.0) and \
                (ammf_orientation_loss > 0.0):

            sum_ammf_loc_loss = eval_ammf_losses[KEY_SUM_ammf_LOC_LOSS]
            sum_ammf_ang_loss = eval_ammf_losses[KEY_SUM_ammf_ANG_LOSS]

            sum_ammf_loc_loss += ammf_localization_loss
            sum_ammf_ang_loss += ammf_orientation_loss

            eval_ammf_losses.update({KEY_SUM_ammf_LOC_LOSS:
                                     sum_ammf_loc_loss})
            eval_ammf_losses.update({KEY_SUM_ammf_ANG_LOSS:
                                     sum_ammf_ang_loss})
            num_valid_regression_samples = \
                eval_ammf_losses[KEY_NUM_VALID_REG_SAMPLES]
            num_valid_regression_samples += 1
            eval_ammf_losses.update({KEY_NUM_VALID_REG_SAMPLES:
                                     num_valid_regression_samples})

        print("Step {}: Eval ammf Loss: "
              "classification {:.3f}, "
              "regression {:.3f}, "
              "total {:.3f}".format(
                global_step,
    ...
    def _update_ammf_box_cls_loc_losses(self,
                                        eval_ammf_losses,
                                        eval_losses,
                                        eval_total_loss,
                                        global_step):
    ...
    def save_proposal_losses_results(self,
                                     ...
                                     global_step,
                                     predictions_base_dir):
        """Helper function to save the p1 loss evaluation results.
        """
        sum_p1_obj_loss = eval_p1_losses[KEY_SUM_p1_OBJ_LOSS]
        sum_p1_reg_loss = eval_p1_losses[KEY_SUM_p1_REG_LOSS]
        sum_p1_total_loss = eval_p1_losses[KEY_SUM_p1_TOTAL_LOSS]
        sum_p1_obj_accuracy = eval_p1_losses[KEY_SUM_p1_OBJ_ACC]

        # Calculate average loss and accuracy
        avg_p1_obj_loss = sum_p1_obj_loss / num_valid_samples
        avg_p1_reg_loss = sum_p1_reg_loss / num_valid_samples
        avg_p1_total_loss = sum_p1_total_loss / num_valid_samples
        avg_p1_obj_accuracy = sum_p1_obj_accuracy / num_valid_samples

        print("Step {}: Average p1 Losses: objectness {:.3f}, "
        ...
        avg_loss_file_path = predictions_base_dir + '/p1_avg_losses.csv'
        ...
        avg_acc_file_path = predictions_base_dir + '/p1_avg_obj_acc.csv'
        with open(avg_acc_file_path, 'ba') as fp:
            np.savetxt(
                fp, np.reshape(
                    [global_step, avg_p1_obj_accuracy],
                    (1, 2)),
                fmt='%d, %.5f')
    ...
    def save_prediction_losses_results(self,
                                       ...
                                       predictions_base_dir,
                                       box_rep):
        """Helper function to save the ammf loss evaluation results.

        Args:
            eval_ammf_losses: A dictionary containing the loss sums
        ...
        sum_ammf_cls_accuracy = \
            eval_ammf_losses[KEY_SUM_ammf_CLS_ACC]

        num_valid_regression_samples = \
            eval_ammf_losses[KEY_NUM_VALID_REG_SAMPLES]

        avg_ammf_cls_loss = sum_ammf_cls_loss / num_valid_samples
        avg_ammf_reg_loss = sum_ammf_reg_loss / num_valid_samples
        avg_ammf_total_loss = sum_ammf_total_loss / num_valid_samples

        if num_valid_regression_samples > 0:
            avg_ammf_loc_loss = \
                sum_ammf_loc_loss / num_valid_regression_samples

            if box_rep in ['box_3d', 'box_4ca']:
                avg_ammf_ang_loss = \
                    sum_ammf_ang_loss / num_valid_regression_samples
        else:
            avg_ammf_loc_loss = 0
            avg_ammf_ang_loss = 0

        avg_ammf_cls_accuracy = sum_ammf_cls_accuracy / num_valid_samples

        # Write summaries
        summary_utils.add_scalar_summary(
            'ammf_losses/classification/classification',
            avg_ammf_cls_loss,
            self.summary_writer, global_step)
        summary_utils.add_scalar_summary(
            'ammf_losses/regression/regression_total',
            avg_ammf_reg_loss,
            self.summary_writer, global_step)

        summary_utils.add_scalar_summary(
            'ammf_losses/regression/localization',
            avg_ammf_loc_loss,
            self.summary_writer, global_step)
        if box_rep in ['box_3d', 'box_4ca']:
            summary_utils.add_scalar_summary(
                'ammf_losses/regression/orientation',
                avg_ammf_ang_loss,
                self.summary_writer, global_step)

        print("Step {}: Average ammf Losses: "
              "cls {:.5f}, "
              "reg {:.5f}, "
              "total {:.5f} ".format(
                global_step,
        ...
        print("Step {}: Average Classification Accuracy: {} ".format(
            global_step, avg_ammf_cls_accuracy))

        # Append to end of file
        avg_loss_file_path = predictions_base_dir + '/ammf_avg_losses.csv'
        if box_rep in ['box_3d', 'box_4ca']:
            with open(avg_loss_file_path, 'ba') as fp:
                np.savetxt(fp,
                           [np.hstack(
        ...
        avg_acc_file_path = predictions_base_dir + '/ammf_avg_cls_acc.csv'
        with open(avg_acc_file_path, 'ba') as fp:
            np.savetxt(
                fp, np.reshape(
                    [global_step, avg_ammf_cls_accuracy],
    ...
    def _create_ammf_losses_dict(self):
        """Returns a dictionary of the losses sum for averaging.
        """
        eval_ammf_losses = dict()
        # Initialize ammf average losses
        eval_ammf_losses[KEY_SUM_ammf_CLS_LOSS] = 0
        eval_ammf_losses[KEY_SUM_ammf_REG_LOSS] = 0
        eval_ammf_losses[KEY_SUM_ammf_TOTAL_LOSS] = 0

        eval_ammf_losses[KEY_SUM_ammf_LOC_LOSS] = 0
        eval_ammf_losses[KEY_SUM_ammf_ANG_LOSS] = 0

        eval_ammf_losses[KEY_SUM_ammf_CLS_ACC] = 0

        # Number of samples that got regressed because
        # they were classified correctly
        eval_ammf_losses[KEY_NUM_VALID_REG_SAMPLES] = 0

        return eval_ammf_losses

    def _create_p1_losses_dict(self):
        """Returns a dictionary of the losses sum for averaging.
        """
        eval_p1_losses = dict()

        # Initialize p1 average losses
        eval_p1_losses[KEY_SUM_p1_OBJ_LOSS] = 0
        eval_p1_losses[KEY_SUM_p1_REG_LOSS] = 0
        eval_p1_losses[KEY_SUM_p1_TOTAL_LOSS] = 0

        eval_p1_losses[KEY_SUM_p1_OBJ_ACC] = 0

        return eval_p1_losses
    ...
    def get_evaluated_ckpts(self,
        ...
        # regardless of model, we are always evaluating p1, but we do
    ...
    def get_cls_accuracy(self,
                         ...
                         eval_p1_losses,
                         global_step):
        """Updates the calculated accuracies for p1 and ammf losses.

        Args:
            predictions: A dictionary containing the model outputs.
            eval_ammf_losses: A dictionary containing all the ammf averaged
                losses.
            eval_p1_losses: A dictionary containing all the p1 averaged
                losses.
            global_step: Current global step that is being evaluated.
        """

        objectness_pred = predictions[p1Model.PRED_MB_OBJECTNESS]
        objectness_gt = predictions[p1Model.PRED_MB_OBJECTNESS_GT]
        objectness_accuracy = self.calculate_cls_accuracy(objectness_pred,
                                                          objectness_gt)

        # get this from the key
        sum_p1_obj_accuracy = eval_p1_losses[KEY_SUM_p1_OBJ_ACC]
        sum_p1_obj_accuracy += objectness_accuracy
        eval_p1_losses.update({KEY_SUM_p1_OBJ_ACC:
                                sum_p1_obj_accuracy})
        print("Step {}: p1 Objectness Accuracy: {}".format(
            global_step, objectness_accuracy))

        if self.full_model:
            classification_pred = \
                predictions[ammfModel.PRED_MB_CLASSIFICATION_SOFTMAX]
            classification_gt = \
                predictions[ammfModel.PRED_MB_CLASSIFICATIONS_GT]
            classification_accuracy = self.calculate_cls_accuracy(
                classification_pred, classification_gt)

            sum_ammf_cls_accuracy = eval_ammf_losses[KEY_SUM_ammf_CLS_ACC]
            sum_ammf_cls_accuracy += classification_accuracy
            eval_ammf_losses.update({KEY_SUM_ammf_CLS_ACC:
                                     sum_ammf_cls_accuracy})

            print("Step {}: ammf Classification Accuracy: {}".format(
                global_step, classification_accuracy))
            '''
            if classification_accuracy< 0.90:#class主要靠IMG,如果它低了，说明img不行了。
                self.model_config.path_drop_probabilities = [0.9, 0]
                print("bqx:use bev2")
            '''
    ...
    def get_p1_proposals_and_scores(self, predictions):
        ...
        Returns:
            proposals_and_scores: A numpy array of shape (number_of_proposals,
                8), containing the p1 proposal boxes and scores.
        """

        top_anchors = predictions[p1Model.PRED_TOP_ANCHORS]
        top_proposals = box_3d_encoder.anchors_to_box_3d(top_anchors)
        softmax_scores = predictions[p1Model.PRED_TOP_OBJECTNESS_SOFTMAX]
    ...
    def get_ammf_predicted_boxes_3d_and_scores(self, predictions,
                                               box_rep):
        ...
        if box_rep == 'box_3d':
            ...
            final_pred_anchors = predictions[
                ammfModel.PRED_TOP_PREDICTION_ANCHORS]
            final_pred_orientations = predictions[
                ammfModel.PRED_TOP_ORIENTATIONS]
            ...
            final_pred_boxes_3d = predictions[
                ammfModel.PRED_TOP_PREDICTION_BOXES_3D]
        ...
        final_pred_softmax = predictions[
            ammfModel.PRED_TOP_CLASSIFICATION_SOFTMAX]
    ...
    def get_ammf_predicted_box_corners_and_scores(self,
                                                  predictions,
                                                  box_rep):

        if box_rep in ['box_8c', 'box_8co']:
            final_pred_box_corners = predictions[ammfModel.PRED_TOP_BOXES_8C]
        elif box_rep in ['box_4c', 'box_4ca']:
            final_pred_box_corners = predictions[ammfModel.PRED_TOP_BOXES_4C]
